...
export class ReportGenerationService {
      ...
      try {
        const doc = new PDFDocument({
          ...
          info: {
            ...
            Author: reportData.config.metadata?.author || 'Kagome Reports',
            ...
            Creator: 'Kagome Reports Application',
            Producer: 'PDFKit'
          }
        });
  ...
  private addReportHeader(doc: PDFKit.PDFDocument, config: ReportConfig): void {
    const companyName = config.branding?.companyName || 'Kagome Reports';
  ...
  private addReportFooter(doc: PDFKit.PDFDocument, reportData: ReportData): void {
    ...
    doc.fillColor('#666666')
       .fontSize(8)
       .text(
         `Generated by Kagome Reports on ${generatedDate}`,
         50,
         doc.page.height - 40,
         { align: 'left' }
       );
